...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):#开始每轮的训练
        ...
        for i_batch,sample_batch in enumerate(train_loader): #获取对应的数据路径
            ...
            m_label_out_ = m_label_out_.transpose(1, 3).transpose(1, 2).contiguous().view(-1,2) #将-1里分成两份刚刚好
    ...
    epoch_item=[]
    for i in range(len(train_loss_items)):#计算论述
        epoch_item.append(i+1)
    # save model
    # unet.train()
    unet.eval()
    torch.save(unet.state_dict(), './CrackForest-dataset-master/save_model_dir/unet_road100trainB_model.pt')
    ...
    plt.plot(epoch_item, train_loss_items)

    plt.savefig("loss100B.png",dpi=300)
    plt.show()
